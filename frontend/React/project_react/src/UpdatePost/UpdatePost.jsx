...
export default function UpdatePost() {
  ...
  React.useEffect(() => {
    ...
    axios.get("/api/productPosts/" + postId)
      .then(res => {
        const oldPost = res.data;
        console.log(oldPost);

        setInputs(values => ({
          ...values,
          ['productPostDescription']: oldPost.description,
          ['weight']: oldPost.product.weight,
          ['pickupDate']: oldPost.expectedPickupDate,
          ['deliveryDate']: oldPost.expectedDeliveryDate,

          ['sourceStreet']: oldPost.source.street,
          ['sourceCity']: oldPost.source.city,
          ['sourceState']: oldPost.source.state,
          ['sourceZip']: oldPost.source.zip,
          ['sourceCountry']: oldPost.source.country,

          ['destStreet']: oldPost.destination.street,
          ['destCity']: oldPost.destination.city,
          ['destState']: oldPost.destination.state,
          ['destZip']: oldPost.destination.zip,
          ['destCountry']: oldPost.destination.country
        }));
      })
      .catch((err) => {
        console.log(err);
        alert("Error getting product post, reason: " + err.response.data.message)
      });
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const resp = await axios.put('/api/productPosts/updateProductPost', data);
      console.log(resp.data);
      alert('Product post updated');
      navigate(urlPaths.myPosts);

    } catch (error) {
      console.log(error);
      alert('Failed to update product post, reason: ' + error.response.data.message);
    }
  }
